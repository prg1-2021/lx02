...
case class TimerWorld2(sec: Int, subsec: Int) extends World() {
  /**
   * draw は画面の描画を担当する関数。
   ...
   **/
  def tick():                World = {
    if (subsec + 1 == 100) { TimerWorld2(sec + 1, 0) }
    else { TimerWorld2(sec, subsec + 1) }
  }
   ...
   * この例では積極的には用いない。
   **/
  def click(p: Pos):         World = { TimerWorld2(sec, subsec) }
  def keyEvent(key: String): World = { TimerWorld2(sec, subsec) }
}

object TimerApp2 extends App {
  // 初期ワールドの構成。ここから世界が始まる。
  TimerWorld2(0, 0).bigBang(50, 50, 0.01)
}
